...
\* --------------------------------------------------------------------------
FixedRoles ==
    { "trace", "decompose_a", "decompose_b",
      "cross_critique_ab", "cross_critique_ba", "disagreement_synthesis",
      "frame_deliberation", "source_ledger",
...
Deps(n) ==
    ...
    CASE n.role = "trace"              -> {}
      \* MULTI-MODEL: the two framing branches are ROOTS and neither depends on
      \* the other — that mutual unreachability IS the blinding, and
      \* BlindBeforeConfrontation checks it rather than trusting a prompt.
      [] n.role = "decompose_a"        -> {}
      [] n.role = "decompose_b"        -> {}
      \* Cross-examination: each critique waits on BOTH branches, so it cannot
      \* start until each has sealed its own artifact. Blindness is enforced by
      \* the edge set, not by asking a model not to look.
      [] n.role = "cross_critique_ab"  -> { Fix("decompose_a"), Fix("decompose_b") }
      [] n.role = "cross_critique_ba"  -> { Fix("decompose_a"), Fix("decompose_b") }
      \* The merge names provenance per result and never resolves by vote.
      [] n.role = "disagreement_synthesis" -> { Fix("cross_critique_ab"),
                                                Fix("cross_critique_ba") }
      [] n.role = "frame_deliberation" -> { Fix("disagreement_synthesis") }
...
Produces(n) ==
    CASE n.role = "trace"              -> { "trace_sidecar" }
      [] n.role = "decompose_a"        -> { "decompose_a_md" }
      [] n.role = "decompose_b"        -> { "decompose_b_md" }
      [] n.role = "cross_critique_ab"  -> { "critique_ab" }
      [] n.role = "cross_critique_ba"  -> { "critique_ba" }
      \* The merged decomposition is what the rest of the DAG consumes, so every
      \* downstream Requires("decompose_md") stays satisfied unchanged.
      [] n.role = "disagreement_synthesis" -> { "decompose_md", "provenance_map" }
...
Requires(n) ==
    CASE n.role = "trace"              -> {}   \* reads live cosmon state, not a DAG artifact
      [] n.role = "decompose_a"        -> {}
      [] n.role = "decompose_b"        -> {}
      [] n.role = "cross_critique_ab"  -> { "decompose_a_md", "decompose_b_md" }
      [] n.role = "cross_critique_ba"  -> { "decompose_a_md", "decompose_b_md" }
      [] n.role = "disagreement_synthesis" -> { "decompose_a_md", "decompose_b_md",
                                                "critique_ab", "critique_ba" }
      [] n.role = "frame_deliberation" -> { "decompose_md", "provenance_map" }
...
PlainRoles ==
    { "trace",
      "decompose_a", "decompose_b", "cross_critique_ab", "cross_critique_ba",
      "disagreement_synthesis",
      "frame_deliberation", "source_ledger", "concept_cards",
...
\* ==========================================================================
\* BlindBeforeConfrontation (multi-model, LOAD-BEARING) — the blinding is
\* STRUCTURAL, not behavioural.
\*
\* The whole comparative claim of a two-provider run rests on the two framing
\* branches having been independent when they sealed their artifacts. Asking a
\* model not to read its neighbour is a behavioural rule, and behavioural rules
\* are exactly what a prompt cannot guarantee. So the blinding is expressed as a
\* property of the EDGE SET, where TLC can discharge it:
\*
\*   (1) neither framing branch is reachable from the other along blocked-by, so
\*       neither can consume the other's artifact — nothing to read, rather than
\*       a request not to look;
\*   (2) every confrontation node depends on BOTH branches, so no cross-critique
\*       or merge can begin before both have sealed.
\*
\* Delete either clause and the property fails, which is its teeth: wiring
\* decompose_b downstream of decompose_a (the obvious "cheap" simplification)
\* would make the run cheaper AND destroy the measurement, silently. Here it is
\* a seal violation instead.
\* ==========================================================================
ConfrontationRoles == { "cross_critique_ab", "cross_critique_ba",
                        "disagreement_synthesis" }

BlindBeforeConfrontation ==
    /\ Fix("decompose_a") \notin ReachDeps(Fix("decompose_b"))
    /\ Fix("decompose_b") \notin ReachDeps(Fix("decompose_a"))
    /\ \A n \in Nodes :
         n.role \in ConfrontationRoles =>
            /\ Fix("decompose_a") \in ReachDeps(n)
            /\ Fix("decompose_b") \in ReachDeps(n)
